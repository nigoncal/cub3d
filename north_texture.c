...
/*   Updated: 2021/04/14 14:02:09 by sylducam         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
 ...
 * 										   - le path et que le fichier soit ouvrable, ces deux choses en utilisant open
 *
 *  
 *
 * a chaque fois s'il y a une erreur, 1 est renvoye, 0 si tout se passe bien
 * si tout s'est bien passe, settings->NO est mis a true
*/
...
static int	right_file(char **elements, cub_settings *settings)
{
	...
	if (((format_check(elements[1], ".xpm")) +
		...
		return (1);
	if ((settings->north_fd = open(elements[1], O_RDONLY)) < 0)
		return (1);
	return (0);
}
