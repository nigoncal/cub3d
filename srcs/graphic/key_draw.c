...
void	draw_rotate_left(t_setup *setup)
{
	if (setup->key.rotate_left == 1)
	{
		...
		double oldplane_x = setup->game.plane_x;
		setup->game.plane_x = setup->game.plane_x * cos(-setup->game.rotspeed) - setup->game.plane_y * sin(-setup->game.rotspeed);
		setup->game.plane_y = oldplane_x * sin(-setup->game.rotspeed) + setup->game.plane_y * cos(-setup->game.rotspeed);
	}
