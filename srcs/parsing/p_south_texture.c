 ...
 *
 * verifie d'abord que l'identifiant n'a pas deja ete rencontre avec le booleen setup->south
 ...
 * si tout s'est bien passe, le path est stocke dans setup->south_texture_path et setup->south est mis a true
*/
...
static int	right_file(t_setup *setup)
{
	...
	fd = 0;
	if (ft_strcmp(setup->elements[0], "SO") != 0)
		return (-1);
