 ...
 *
 * verifie d'abord que l'identifiant n'a pas deja ete rencontre avec le booleen setup->sprite
 ...
 * si tout s'est bien passe, le path est stocke dans setup->sprite_texture_path et setup->sprite est mis a true
*/
...
static int	right_file(t_setup *setup)
{
	...
	fd = 0;
	if (ft_strcmp(setup->elements[0], "S") != 0)
		return (-1);
...
void	p_sprite_texture(char *line, t_setup *setup)
{
	...
	setup->sprite_texture_path = ft_strdup(setup->elements[1]);
	setup->sprite = true;
