 ...
 *
 * verifie d'abord que l'identifiant n'a pas deja ete rencontre avec le booleen cub_sets->S
...
static int	right_file(char **elements, t_settings *cub_sets)
{
	...
	cub_sets->sprite_fd = fd; // a virer, juste pour les test
	if (fd >= 0)
		return (-1);
...
int			sprite_texture(char *line, t_settings *cub_sets)
{
	...
	if (right_amount(elements, cub_sets) == -1)
		return (-1);
	cub_sets->sprite_texture_path = ft_strdup(elements[1]);
	cub_sets->S = true;
	free(elements);
	return (0);
}
